...
chrome.contextMenus.onClicked.addListener(async (info) => {
  ...
  const task = taskFromInput(info.selectionText.trim().slice(0, 300), 'page');
  if (!task) return;
  await addTask(task);
  await refreshBadge();
...
async function fireDueReminders() {
  ...
  for (const t of tasks) {
    ...
    // Don't dump a week of missed reminders on someone reopening Chrome.
    const stale = now - t.due > 12 * 60 * 60 * 1000;
    t.notified = true;
    dirty = true;
    if (stale) continue;
